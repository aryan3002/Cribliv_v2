...
/**
 * The landmark hub page's render, extracted out of `page.tsx` so the admin
...
export async function LandmarkHubView({
  params,
  revalidate,
  allowUnlisted = false
}: {
  ...
  const [landmark, listingsBundle] = await Promise.all([
    fetchLandmark(params.citySlug, params.landmark, { revalidate }),
    fetchLandmarkListings(params.citySlug, params.landmark, {
      radiusKm: 2,
      limit: 24,
      revalidate
    })
