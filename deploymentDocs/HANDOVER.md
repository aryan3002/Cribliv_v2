...
- [ ] **Wire payment env vars:** `PAYMENT_PROVIDER_KEY`, `RAZORPAY_WEBHOOK_SECRET`, `UPI_WEBHOOK_SECRET`. Currently the payments module crashes if these are missing at first call; add as secrets, redeploy, test webhook locally with `ngrok` first then update Razorpay dashboard to point at production URL.
