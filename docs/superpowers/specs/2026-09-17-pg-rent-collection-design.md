...
| D18 | **Final-period re-proration is owner-tapped, never automatic.** Notice / move-out puts a "Rahul leaves on X — charge only till X (₹Y)?" row in the queue (on-screen words: §8.10); nothing changes a bill until the owner taps; a cancelled notice removes the row.                                                                                                                                                                           | §19 #5 — `notice_end_date` is tenant-set and format-validated only; it must not drive a paid-invoice reduction.                                                                       |
...
- `serve_notice` (tenant), `operator_move_out_request`, `confirm_move_out`, `direct_move_out` → run generation for that assignment (so a not-yet-created final period is created, already cut per §5.2), then: if `prorate_move_out` is on and the leave date (`notice_end_date` or `move_out_date`) is earlier than the end of an **already-issued** period, write `invoice.final_reprorate_suggested {leave_on, from_paise, to_paise}` on that invoice. The queue shows **"Rahul leaves on 15 Oct — charge October only till 15 Oct (₹4,355)?"** → **Charge till 15 Oct** (runs `invoice.reprorated`, via §6.6 if the invoice is paid, the extra shown as "extra paid — adjust in final hisaab") or **Keep full month**. A leave date that is before `move_in_date` or before the period start is refused as a suggestion (the row says "check the notice date").
- `cancel_move_out` / `cancel_notice` (back to `active`) → an unactioned suggestion disappears; an _applied_ re-proration is not undone automatically — the row becomes "Rahul is staying — charge full October again (₹9,000)?" → **Charge full month** (internally "Restore"; puts the rent line and `period_end` back to their pre-proration values — kept in the rent line's `meta.reprorated` — `invoice.line_updated {reason:'reprorate_restored'}`, then re-applies the assignment's unallocated credit to this invoice FIFO in the same transaction so the money that §6.6 released comes straight back; rent stays a rent line so analytics never see a synthetic adjustment).
...
| `partially_paid` invoice cancelled                               | Allocations released to credit; tenant sees "₹X kept as extra paid — adjusts in your next bill" |
...
### 8.10 Words on screen — plain language (owner decision 2026-09-24)

The people using this are PG operators and tenants in India, not accountants. **Engineering names stay in the code, the API and the events; they never appear on screen.** Every owner and tenant string in the web slices (`apps/web/lib/i18n.ts`, en + hi) uses the words below. When a screen needs a concept that isn't listed here, write it the same way: the everyday word an operator would say aloud, with a number or a date in it.

| Engineering name (code/API only)                  | Owner / tenant sees — English                                     | Hindi                                                                   |
| ------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------- |
| invoice                                           | Bill                                                              | बिल                                                                     |
| receipt                                           | Receipt                                                           | रसीद                                                                    |
| re-prorate / final re-proration suggestion (§5.8) | Charge only till leaving date — button "Charge till 15 Oct"       | जाने की तारीख़ तक का किराया — बटन "15 अक्टूबर तक का किराया लें"         |
| restore (§5.8)                                    | Charge full month again — button "Charge full month"              | पूरे महीने का किराया फिर से — बटन "पूरे महीने का किराया लें"            |
| prorated line, `proration_factor`                 | Part month · 15 of 31 days                                        | महीने के 15 दिन (31 में से)                                             |
| unallocated credit, excess de-allocation (§6.6)   | Extra paid · ₹1,500 — adjusts in the next bill                    | ज़्यादा जमा · ₹1,500 — अगले बिल में कट जाएगा                            |
| allocation, FIFO                                  | never shown; "Added to September rent"                            | "सितंबर के किराये में जोड़ा"                                            |
| claim / `pending_confirmation`                    | Tenant says paid — check and confirm                              | किरायेदार ने भुगतान बताया — जाँचकर पक्का करें                           |
| reverse (payment)                                 | Undo this payment                                                 | यह भुगतान हटाएँ                                                         |
| settle / settlement statement (§6.11)             | Final hisaab (deposit and dues)                                   | फ़ाइनल हिसाब (डिपॉज़िट और बकाया)                                        |
| forfeit booking amount (§6.12)                    | Keep booking amount                                               | बुकिंग राशि रखें                                                        |
| backfill                                          | Add an old payment                                                | पुराना भुगतान जोड़ें                                                    |
| grace days                                        | Late fee starts N days after the due date                         | देय तारीख़ के N दिन बाद लेट फ़ीस                                        |
| `prorate_move_out` setting                        | When someone leaves mid-month, offer to charge only till that day | महीने के बीच में जाने पर, सिर्फ़ उस दिन तक का किराया लेने का विकल्प दें |

**Change-log lines** (owner ledger and the tenant's "This bill" timeline):

| Event                                                                   | English                                                        | Hindi                                                               |
| ----------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------- |
| `invoice.reprorated`                                                    | September now charged till 15 Sep (₹4,500) — leaving on 15 Sep | सितंबर का किराया अब 15 सितंबर तक (₹4,500) — 15 सितंबर को जा रहे हैं |
| `invoice.line_updated {reason:'reprorate_restored'}`                    | September back to full month (₹9,000) — staying on             | सितंबर फिर से पूरे महीने का (₹9,000) — रुक रहे हैं                  |
| `invoice.cancelled {reason:'restore_absorbed'}` (on the 16–30 Sep bill) | 16–30 Sep bill merged back into September — staying on         | 16–30 सितंबर का बिल सितंबर के बिल में जोड़ दिया — रुक रहे हैं       |
| `invoice.excess_deallocated`                                            | ₹X kept as extra paid — adjusts in the next bill               | ₹X ज़्यादा जमा रखा — अगले बिल में कट जाएगा                          |

When one "Charge full month" tap merges a bill and re-applies the money, show **one line per bill** (the two rows above). Hide the intermediate "kept as extra paid" events that the same transaction writes: the money moved out and straight back, so the net is zero and showing it only confuses.

**Error codes the web must translate:**

- `restore_gap_edited` → "The 16–30 Sep bill has a late fee or an extra charge. Remove it first, then charge the full month." / "16–30 सितंबर के बिल में लेट फ़ीस या अतिरिक्त चार्ज है। पहले उसे हटाएँ, फिर पूरे महीने का किराया लें।"
- `invoice_cancelled` → "This bill is cancelled — nothing to change." / "यह बिल रद्द है — इसमें कुछ बदलना नहीं है।"
- `duplicate_invoice` → "This bill was already created." / "यह बिल पहले ही बन चुका है।"
...
3. **This invoice** — lines as the owner sees them, invoice number, period, tenant-visible change log ("Electricity ₹896 added · 2 Oct"; "₹1,000 kept as extra paid · 4 Oct" for §6.6).
