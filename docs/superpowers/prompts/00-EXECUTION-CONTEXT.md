...
- Migrations run via `infra/migrations/run-migrations.js`: filename MUST match `^\d+_.*\.sql$`, MUST NOT contain the substring `rollback`, applied in lexical order, each in its own `BEGIN…COMMIT`, tracked in `schema_migrations`. Before creating a new migration, fetch/check current `origin/master` and use the next free number after master. Companion rollback file: `00NN_name.rollback.sql`. Be idempotent (`IF NOT EXISTS`, `DO $$ … EXCEPTION WHEN duplicate_object THEN null; END $$`). Reuse the existing `trigger_set_updated_at()` function (defined in `0001_init.sql`).
