...
### Slice 1.1 — Migration `0055_pg_manage_requests.sql` (+ rollback)

- Create `infra/migrations/0055_pg_manage_requests.sql` using the **verbatim DDL in plan §4** (`0055` block): enum `pg_manage_request_status`; `ALTER TABLE pg_properties ADD manage_enabled/layout_status/managed_activated_at`; table `pg_manage_requests` (with the two partial unique indexes `uq_pg_manage_pending_per_listing`, `uq_pg_manage_approved_per_listing`); shell table `pg_manage_payment_orders`; the `payment_order_id` FK; the `set_updated_at` trigger.
- Create `infra/migrations/0055_pg_manage_requests.rollback.sql` that drops them in reverse dependency order.
